...
function Get-CMLog {
    ...
    function _getAndCacheLogDescription {
        ...
        . "$PSScriptRoot\Get-LogDescriptionAndCache.ps1"
    ...
    function _getLogDescription {
        ...
        if ($script:logDescription) {
            ...
            $logName | ForEach-Object {
                ...
                $wantedLogDescription = $script:logDescription | Where-Object LogName -Match "^$lName\.log$|^$lName[-_].+\.log$"

                if ($wantedLogDescription) {
                    # for better readibility output as string
                    $wantedLogDescription | ForEach-Object {
                        $_.LogName
                        " - " + $_.Description
                        ""
                    }
    ...
    function _openLog {
        ...
        foreach ($lName in $logName) {
            ...
            $logFolder = $logDetails | Where-Object logName -Contains $lName | Select-Object -ExpandProperty logFolder
            ...
            foreach ($lFolder in $logFolder) {
                ...
                try {
                    ...
                    $availableLogs = Get-ChildItem $lFolder -Force -File -ErrorAction Stop | Where-Object Name -Match "$regEscLog\.log?$|$regEscLog-[A-Z0-9-]+\.log?$|$regEscLog`_.+@.+\.log?$|$regEscLog-[0-9-]+\.log?$|CMG-.+$regEscLog" | Sort-Object LastWriteTime -Descending | Select-Object -ExpandProperty FullName
                }
                catch {
                ...
                if ($availableLogs) {
                    ...
                    $wantedLog = $availableLogs | Where-Object { $_ -match "\.log$" } | Select-Object -First 1
            ...
            if (!$wantedLog) {
                Write-Warning "No '$lName' logs found in $(($logFolder | ForEach-Object {"'$_'"} ) -join ', ')"
            }
        }
        ...
        if ($logPath) {
            if ($viewer -and $viewer -match "CMLogViewer\.exe$") {
                ...
                $quotedLog = ($logPath | ForEach-Object {
                        "`"$_`""
                    }) -join " "
    ...
    if ($area) {
        $result = $areaDetails.GetEnumerator() | Where-Object Key -EQ $area | Select-Object -ExpandProperty Value

        if (!$result) { 
            throw "Undefined area '$area'" 
        }
    ...
    if ($result.writeHost) { 
        Write-Host ("`n" + $result.writeHost + "`n") 
    }

    if ($result.writeWarning) { 
        Write-Warning $result.writeWarning 
    }
...
Get-CMLog -computerName 'LP532' -logName ccmsetup
